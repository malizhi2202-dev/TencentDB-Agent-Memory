...
- `@.specs/CONTEXT.md`
- `@code-kit/reference/ui-aesthetics.md`（决策框架）
- `@code-kit/reference/ui-anti-patterns.md`（反 AI-slop 清单）
...
- `.specs/<id>/UI-DESIGN.md`（必填）
  - 使用 `@code-kit/templates/UI-DESIGN.md` 模板
...
- [ ] **🛡️ G2a 门禁已跑**：4 领域专家投票已执行，结果已记录，全票/多数通过后才触发下一步

## 🛡️ 专家团门禁 · G2a UI 设计门（强制 · R9/R13）

UI-DESIGN.md 完成后，**必须**启动该阶段的 4 领域专家审核。不通过不得进入任务拆分。

### G2a 专家团构成（UI 设计阶段专有）

| 角色 | 核心关注点 |
|---|---|
| 🟫 **资深UI设计师** | 调性是否纯粹、视觉层次是否清晰、信息排版是否可扫读、是否避免了「AI slop」感。参考：原研哉（MUJI 极简）、Jony Ive（Apple 工业）、Saul Bass（图形张力）、Massimo Vignelli（纽约地铁图·信息秩序）|
| 🟦 **资深用户体验官** | 交互是否直觉、信息架构是否清晰、状态是否完备、可用性是否有硬伤、用户流是否最短路径 |
| 🟩 **前端架构师** | 组件架构是否合理、design tokens 是否可落地、实现成本是否可控 |
| 🔴 **无障碍专家** | WCAG 合规、色盲/低视力可用、键盘可达、读屏器友好、`prefers-reduced-motion` 响应 |

### 投票问题

> 「UI-DESIGN.md 视觉方案是否通过？调性是否纯粹、美学是否达标、交互是否可用、组件是否可落地、无障碍是否覆盖？可以进入任务拆分吗？」
...
🗳️ G2a UI 设计门: UI-DESIGN.md 是否通过？
   ...
   🟫 资深UI设计师: ✅/❌ <理由（关注：调性纯粹度、视觉层次、信息排版、AI slop 规避）>
   🟦 资深用户体验官: ✅/❌ <理由（关注：交互直觉、信息架构、状态完备、用户流最短路径）>
   🟩 前端架构师: ✅/❌ <理由（关注：组件架构、design tokens 落地、实现成本）>
   🔴 无障碍专家: ✅/❌ <理由（关注：WCAG、色盲、键盘、读屏器、reduced-motion）>
...
| 结果 | 动作 |
|---|---|
| 全票通过（4/4） | ✅ 自动进入 `3-task.md` |
| 多数通过（3/4） | ✅ 自动进入，记录反对意见到 UI-DESIGN.md 末尾 |
| 平票（2/2）或争议 | ⚠️ 暂停，列出分歧点提交用户裁决 |
| 全票反对 | ⚠️ 暂停，列出所有反对理由，回到 UI 设计修改 |

### 通过后

进入任务拆分。**禁止**在全票/多数通过后还问「要继续吗？」（R13.4）。

## 触发下一步

门禁通过后：`@code-kit/prompts/3-task.md` —— 拆任务时把 design tokens 物化到代码（CSS variables / theme 文件）作为第一批任务