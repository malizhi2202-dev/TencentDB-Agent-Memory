...
- [ ] **🛡️ G2 门禁已跑**：4 角色投票已执行，结果已记录，全票/多数通过后才触发下一步

## 🛡️ 专家团门禁 · G2 方案门（强制 · R9/R13）

DESIGN.md 完成后，**必须**启动该阶段的 4 领域专家审核。不通过不得进入下一阶段。
...
| 角色 | 核心关注点 |
|---|---|
| 🟫 **架构师** (M) | 技术方案合理性、ADR 决策质量、模块边界、架构对齐 |
| 🟦 **研发负责人** | 实现可行性、技术栈匹配度、工时预估、潜在实现陷阱 |
| 🟩 **领域专家** | 本项目领域的资深内行，判断业务逻辑/数据流/领域建模是否正确 |
| 🔴 **安全审计师** | 安全设计审查、威胁建模、认证鉴权方案、数据流安全 |

### 投票问题

> 「DESIGN.md 技术方案是否可行？技术栈选择、ADR、风险缓解是否充分？从架构、工程、领域、安全四个维度看，可以进入任务拆分吗？」
...
🗳️ G2 方案门: DESIGN.md 是否通过？
   ...
   🟫 架构师(M): ✅/❌ <理由（关注：架构对齐、ADR 合理性、模块边界）>
   🟦 研发负责人: ✅/❌ <理由（关注：实现可行性、技术栈匹配、工时评估）>
   🟩 领域专家: ✅/❌ <理由（关注：业务逻辑正确性、领域建模、数据流）>
   🔴 安全审计师: ✅/❌ <理由（关注：安全设计、威胁建模、认证鉴权）>
...
|---|---|
| 全票通过（4/4） | ✅ 自动进入下一阶段 |
| 多数通过（3/4） | ✅ 自动进入，记录反对意见到 DESIGN.md 末尾 |
| 平票（2/2）或争议 | ⚠️ 暂停，列出分歧点提交用户裁决 |
| 全票反对 | ⚠️ 暂停，列出所有反对理由，回到设计修改 |
