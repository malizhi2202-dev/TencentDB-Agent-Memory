...
- [ ] 跳过的轮次都有理由
- [ ] **🛡️ 测试门禁已跑**：4 领域专家投票已执行，结果已记录，全票/多数通过后才触发下一步

## 🛡️ 专家团门禁 · 测试门（强制 · R9/R13）

TEST.md 完成后，**必须**启动该阶段的 4 领域专家审核。不通过不得进入 6-review。

### 测试门专家团构成（测试阶段专有）

| 角色 | 核心关注点 |
|---|---|
| 🟫 **资深测试工程师（各领域）** | 5 轮测试是否完整执行、覆盖率是否达标、边界用例是否充分、测试质量 6 维（T1~T6）是否通过。具备多领域测试经验：功能/性能/安全/兼容/可观测，能识别各领域特有的测试盲区 |
| 🟦 **研发负责人** | 测试是否验证了真正的实现风险点、mock 是否合理、性能测试基线是否真实、是否有遗漏的关键路径 |
| 🟩 **高级产品经理** | 测试是否覆盖了核心用户场景、AC 覆盖是否与业务价值对齐、UAT 脚本是否反映真实使用路径、是否遗漏产品边界场景 |
| 🔴 **资深用户体验官** | UAT 是否覆盖了真实用户操作流、可访问性测试是否充分（WCAG/色盲/键盘）、界面状态是否完备、错误提示是否友好、操作流是否最短路径 |
...
🗳️ 测试门: TEST.md 是否通过？
   ...
   🟫 资深测试工程师: ✅/❌ <理由（关注：5 轮完整性、覆盖率、边界用例、T1~T6 质量、领域盲区）>
   🟦 研发负责人: ✅/❌ <理由（关注：实现风险覆盖、mock 合理性、性能基线、遗漏路径）>
   🟩 高级产品经理: ✅/❌ <理由（关注：AC 覆盖与业务价值对齐、UAT 场景真实性、产品边界）>
   🔴 资深用户体验官: ✅/❌ <理由（关注：UAT 操作流、可访问性、界面状态完备、错误提示、最短路径）>

   结果: N/4 → <全票通过 ✅ 自动进入 6-review / 多数通过 ✅ / 平票 ⚠️ 提交用户>
```
...
| 结果 | 动作 |
|---|---|
| 全票通过（4/4） | ✅ 自动进入 `6-review.md` |
| 多数通过（3/4） | ✅ 自动进入，记录反对意见到 TEST.md 末尾 |
| 平票（2/2）或争议 | ⚠️ 暂停，列出分歧点提交用户裁决 |
| 全票反对 | ⚠️ 暂停，回对应阶段修复（🔴 问题先回 4-dev，测试缺失先补测） |

### 通过后

进入 review 阶段。**禁止**在全票/多数通过后还问「要继续吗？」（R13.4）。
