...
Forge 不应该成为 code-kit 的依赖。

- 没装 Forge：code-kit 行为不变，继续按 `@code-kit/GO.md` 和 `prompts/*.md` 使用。
- 装了 Forge：AI 可以把 code-kit 的阶段、change-id、task-id 和风险信息写入 Forge state/routing，供 runtime guard 使用。
- Forge 缺失或不可用：必须 fail-open 到纯 markdown 流程，不应阻断 code-kit。
...
只有前 3 个存在时，说明项目使用 code-kit；同时存在 Forge hooks 时，说明可以启用 Forge runtime adapter。
...
Forge adapter 不要求 code-kit 改变产物格式。推荐只写一条轻量 routing event：
...
小改动、一次性脚本、纯讨论和非 Claude Code 环境，继续用纯 markdown code-kit 即可。