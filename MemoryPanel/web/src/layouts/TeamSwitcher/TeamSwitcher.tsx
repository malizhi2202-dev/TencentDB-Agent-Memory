...
export function TeamSwitcher({ userRole }: { userRole: TeamRole | null }) {
  ...
  function pick(team_id: string, close: () => void) {
    ...
    close();
    // 全局页面（用户管理、API Key 管理）切换 team 时，跳转到该 team 的成员管理
    const globalPages = ['/admin/users', '/team/api-keys'];
    if (globalPages.some((p) => location.pathname.startsWith(p))) {
  ...
  return (
    <Dropdown
      ...
      button={
        <button
          type="button"
          className="_memory-team-switcher-title"
          title={active?.name ?? t('teamSwitcher.selectTeam')}
        >
          <span className="_memory-team-switcher-title-text">
