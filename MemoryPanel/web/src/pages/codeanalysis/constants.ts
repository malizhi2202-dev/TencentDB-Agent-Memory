/**
 * 代码分析工具定义 —— 17 个分析工具（源自 GitNexus，代码级集成），按类别分组。
 * 每个类别对应左侧边栏一个功能菜单项。
 */
...
export interface AnalysisToolDef {
  ...
  label: string;
  description: string;
  params: AnalysisToolParam[];
}

export interface AnalysisToolCategory {
  key: string;
  label: string;
  tools: AnalysisToolDef[];
}

export const ANALYSIS_TOOL_CATEGORIES: AnalysisToolCategory[] = [
  {
    key: 'search',
    label: '搜索与查询',
    tools: [
  ...
  },
  {
    ...
    tools: [
      ...
      },
      {
        name: 'check',
        label: '代码检查',
        description: '代码质量检查（TODO/FIXME/console.log/any 类型等规则）',
        params: [
          { name: 'file', label: '按文件过滤', type: 'string', required: false },
