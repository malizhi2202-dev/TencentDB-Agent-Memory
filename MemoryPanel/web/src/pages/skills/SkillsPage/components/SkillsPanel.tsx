...
import { UserIcon, DeleteIcon } from 'tea-icons-react';
...
function SkillOwnerTag({ userId, isCurrentUser }: { userId: string; isCurrentUser: boolean }) {
  ...
  return (
    <AssetBadge
      ...
      title={t('skills.ownerTag.title', { name: displayName || userId, id: userId })}
    >
      {displayName || userId}
...
export default function SkillsPanel({
  ...
  return (
    <div className="_memory-skills-body">
      ...
      <AssetPageHeader
        ...
        scope={
          <Segment
            ...
            options={(['team', 'project', 'agent', 'user'] as Tab[]).map((t2) => ({
              value: t2,
        ...
        agent={
          tab === 'agent' ? (
            ...
            <Select
              ...
              placeholder={t('skills.project.placeholder')}
            ...
            <Select
              ...
              placeholder={t('skills.user.ownerPlaceholder')}
              options={[
                { value: '', text: t('skills.user.self') },
                ...allUsers
        ...
        actions={
          <>
            {(() => {
              ...
              const tooltip =
                tab === 'agent'
                  ? t('skills.fork.tooltip.agent')
                  : tab === 'project'
                    ? t('skills.fork.tooltip.project')
                    : tab === 'user'
                      ? t('skills.fork.tooltip.user')
                      : !selectedSkillId
                        ? t('skills.fork.tooltip.team.empty')
                        : undefined;
              // fork 仅团队资产 tab 可用（agent/user/project 维度无需 fork）
              return tab !== 'team' ? null : (
      ...
      <AssetSplitLayout
        ...
        sidebar={
          <AssetListPanel
            title={
              <>
                ...
                {tab === 'project' && selectedProject && (
                  <span className="_alp-title-suffix">
                    {' '}
                    · {projects.find((p) => p.project_id === selectedProject)?.name ?? selectedProject}
                  </span>
                )}
                {tab === 'user' && selectedOwner && (
                  <span className="_alp-title-suffix">
                    {' '}
                    · {allUsers.find((u) => u.user_id === selectedOwner)?.username ?? selectedOwner}
                  </span>
                )}
              </>
            }
            ...
            emptyText={
              tab === 'agent' && !selectedAgent
                ? t('skills.empty.agent.noAgent')
                : tab === 'agent'
                  ? t('skills.empty.agent.hasAgent', { agent: selectedAgent })
                  : tab === 'project'
                    ? t('skills.empty.project')
                    : tab === 'user'
                      ? t('skills.empty.user')
                      : t('skills.empty.team')
            }
      ...
      {/* Modals (only for team/agent tabs) */}
      {showImport && (
        // 导入弹窗在 team / agent 两个 tab 都能打开。
        // target 是数据面语义：fixed = agent fixed assets（所有 skill 都归属于某个具体 agent）。
        // agentId 仅在 agent tab 下作为默认归属带入；team tab 下传 undefined，
        ...
        <ImportSkillDialog
          ...
          userId={myUserId}
          agents={teamAgents}
          agentId={tab === 'agent' ? selectedAgent : undefined}
