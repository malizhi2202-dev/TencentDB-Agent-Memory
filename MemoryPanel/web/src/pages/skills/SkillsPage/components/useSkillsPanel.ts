...
export function useSkillsPanel() {
  ...
  // 用户维度（user tab）：admin 可选看「任意用户」的资产；空 = 自己。
  ...
  // admin 用户维度：拉取实例级全量用户，供 mine tab 的「按用户切换」选择器使用。
  useEffect(() => {
    if (!isAdmin) {
      setAllUsers([]);
      return;
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      if (tab === 'team') {
        ...
        // project 维度：按协作轴看「该项目下的资产」（跨 team，caller 须 project 可见）。
        if (!selectedProject) {
          if (seq !== refreshSeqRef.current) return;
          setSkills([]);
          setVisibilityMap({});
        } else {
          const assets = await assetsApi.listByProject(selectedProject, { asset_type: 'skill' });
          if (seq !== refreshSeqRef.current) return;
          const visMap: Record<string, Asset['visibility']> = {};
          for (const a of assets) visMap[a.asset_id] = a.visibility;
          const items: SkillSummary[] = assets.map((a) => ({
            skill_id: a.asset_id,
            name: a.name,
            description: a.description ?? '',
            version: a.version ?? 1,
        ...
        // 用户维度 tab：admin 可选看任意用户，普通用户只看自己。
  ...
  }, [tab, selectedAgent, activeTeamId, myUserId, selectedOwner, selectedProject]);
  ...
  useEffect(() => {
    // key 中只有 agent / project tab 才纳入对应的次级选择器 —— team/user tab 的数据源
    // asset/list-accessible 与选中 agent/project 无关。若把 selectedAgent 纳入 team 的 key，
    // teamAgents 异步加载完后 selectedAgent 会从 '' 变成首个 agent，导致 key 变化、
    // 再触发一次**完全重复**的 list-accessible（进页面即多打一次接口）。
    const scopeKey =
      tab === 'agent' ? selectedAgent : tab === 'project' ? selectedProject : '';
    const key = `${activeTeamId}|${tab}|${scopeKey}`;
    if (fetchKeyRef.current === key) return;
    fetchKeyRef.current = key;
    void refresh();
  }, [activeTeamId, tab, selectedAgent, selectedProject, refresh]);
