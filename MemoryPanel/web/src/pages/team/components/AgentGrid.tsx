...
import { useAgentSpaceCounts } from './useAgentAssets';
...
export default function AgentGrid({
  ...
  const { t } = useTranslation();
  const { spaceCounts, spaceLoading } = useAgentSpaceCounts(agents);
  ...
  // 项目维度：拉取当前用户可访问的 project，供 project 筛选下拉使用。
  useEffect(() => {
    let cancelled = false;
    projectsApi
      .list()
  ...
  const filteredAgents = useMemo(() => {
    ...
    return agents.filter((agent) => {
      ...
      if (projectFilter && (agent.project_id ?? '') !== projectFilter) return false;
  ...
  }, [agents, keyword, ownerFilter, projectFilter]);
  ...
  function renderAssets(agent: StoreAgent, countsLoading = false) {
    ...
    const spaces = spaceCounts[agent.agent_id] ?? 0;
    return (
      <div className="_memory-agents-stats">
        ...
        <Mounted label="memory_spaces" count={spaces} loading={spaceLoading} />
      </div>
    );
  }
          ...
          right={
            <div className="_memory-agents-toolbar">
              ...
              <Select
                ...
                appearance="button"
                clearable
                placeholder={t('agentGrid.projectPlaceholder')}
              ...
              {canSeeAllAgents && (
                <Select
                  ...
                  appearance="button"
                  clearable
                  placeholder={t('agentGrid.ownerPlaceholder')}
                  options={ownerOptions.map((ownerId) => ({ value: ownerId, text: resolveUserName(ownerId) }))}
                  matchButtonWidth
                />
              )}
              <Segment
              ...
              render: (agent: StoreAgent) => {
                ...
                return (
                  <span className="_memory-agents-list-assets">
                    skills×{counts.skills} · code_graph×{counts.code_graph} · llm_wiki×{counts.llm_wiki} · chat_memory×{counts.chat_memory} · memory_spaces×{spaceLoading ? '…' : spaces}
                  </span>
                );
              },
            },
