...
import { Alert, Button, Copy, Form, Input, Modal, Segment, Select, Switch, Tag, StatusTip } from 'tea-component';
...
export function AddMemberDialog({
  ...
  useEffect(() => {
    ...
    usersApi
      ...
      .then((list) => {
        if (cancelled) return;
        // 过滤掉当前用户和已在 team 中的成员
        const existingUserIds = new Set(team.members.map((m) => m.user_id));
        setUserList(list.filter((u) => u.user_id !== currentUser && !existingUserIds.has(u.user_id)));
      })
      .catch(() => {
  ...
  }, [team.team_id, currentUser]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // 是否自动创建个人 Agent（agent_id = user_id）：默认开启，关闭则只加成员不建 agent
  const [createDefaultAgent, setCreateDefaultAgent] = useState(true);
  ...
  async function submitExisting() {
    ...
    setError(null);
    try {
      await membersApi.add(team.team_id, { user_id: id, role, create_default_agent: createDefaultAgent });
      invalidateBackendCache();
      onClose();
    } catch (err) {
      tea.notify.error(err);
  ...
  async function submitNew() {
    ...
    try {
      ...
      // Step 3: 自动加入当前 team
      await membersApi.add(team.team_id, { user_id: created.user_id, role, create_default_agent: createDefaultAgent });
      invalidateBackendCache();
  ...
  return (
    ...
    >
          ...
          <div>
            {userListLoading ? (
              ...
              <Select
                ...
                onSearch={(keyword) => setUserSearch(keyword ?? '')}
                ...
                options={userList
                  ...
                  .map((u) => ({
                    value: u.user_id,
                    text: u.username?.trim() ? `${u.username.trim()} (${u.user_id})` : u.user_id,
                  }))}
              />
            )}
      ...
      <Form.Item label={t('addMember.createDefaultAgent')}>
        <div>
          <Switch
            value={createDefaultAgent}
            onChange={(v) => {
              setCreateDefaultAgent(v);
              setError(null);
            }}
          />
          <div className="_memory-field-hint">{t('addMember.createDefaultAgent.hint')}</div>
        </div>
      </Form.Item>
