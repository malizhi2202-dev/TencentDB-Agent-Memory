...
import { type Agent as StoreAgent, invalidateBackendCache, writeAgentUiMeta, EMPTY_AGENT_LLM, type AgentLlmConfig } from '@/services';
...
import { LightField, CollapseGroup, AssetCheckList, AgentLlmFields } from './shared';
...
export default function AgentEditDialog({
  ...
  const [llm, setLlm] = useState<AgentLlmConfig>({ ...EMPTY_AGENT_LLM, ...(agent.llm ?? {}) });
  ...
  const llmChanged =
    JSON.stringify(llm) !== JSON.stringify({ ...EMPTY_AGENT_LLM, ...(agent.llm ?? {}) });
  const agentChanged = promptChanged || assetsChanged || llmChanged;
    ...
    <Modal visible caption={t('agentEdit.caption')} size="l" onClose={onClose}>
      <Modal.Body>
        <div className="_memory-form-stack">
          ...
          <AgentLlmFields value={llm} onChange={setLlm} disabled={saving} />
