...
import { Checkbox, Input, Select } from 'tea-component';
...
/**
 * Agent 的 LLM 模型配置表单：提供方 / 模型名 / 输入输出协议。
 * 空 provider / 空 model 表示沿用全局默认（与既有 resolveLlmConfig 链路保持一致）。
 */
export function AgentLlmFields({
  ...
  onChange: (next: AgentLlmConfig) => void;
  disabled?: boolean;
}) {
  const { t } = useTranslation();
  const providerOptions = [
    { value: '', text: t('agentLlm.providerDefault') },
    { value: 'openai', text: 'OpenAI' },
    { value: 'anthropic', text: 'Anthropic' },
    { value: 'deepseek', text: 'DeepSeek' },
    { value: 'zhipu', text: t('agentLlm.provider.zhipu') },
    { value: 'qwen', text: t('agentLlm.provider.qwen') },
    { value: 'moonshot', text: t('agentLlm.provider.moonshot') },
    { value: 'custom', text: t('agentLlm.provider.custom') },
  ];
  const protocolOptions = [
    { value: 'openai', text: t('agentLlm.protocol.openai') },
    { value: 'anthropic', text: t('agentLlm.protocol.anthropic') },
  ];

  return (
    <div className="_memory-form-stack">
      <div className="_memory-modal-description">{t('agentLlm.section')}</div>
      <LightField label={t('agentLlm.providerLabel')} hint={t('agentLlm.providerHint')}>
        <Select
          appearance="button"
          size="full"
      ...
      <LightField label={t('agentLlm.modelLabel')} hint={t('agentLlm.modelHint')}>
        <Input
          size="full"
          value={value.model}
          onChange={(v) => onChange({ ...value, model: v })}
          placeholder={t('agentLlm.modelPlaceholder')}
          disabled={disabled}
        />
      </LightField>
      <LightField label={t('agentLlm.protocolLabel')} hint={t('agentLlm.protocolHint')}>
        <Select
          appearance="button"
          size="full"
          value={value.protocol}
          onChange={(v) => onChange({ ...value, protocol: v as AgentLlmConfig['protocol'] })}
