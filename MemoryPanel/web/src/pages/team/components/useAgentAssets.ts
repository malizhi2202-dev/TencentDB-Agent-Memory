...
/**
 * 批量拉取各 agent 的「记忆空间」挂载数量（内核 /v3/memory/space/list）。
 * 与 useAgentMountedCounts 不同：space/list 是单 agent 接口，这里并发查每个 agent。
 * 单个失败降级为 0，不影响整体。
 */
export function useAgentSpaceCounts(
  agents: StoreAgent[],
): { spaceCounts: Record<string, number>; spaceLoading: boolean } {
  ...
  useEffect(() => {
    ...
    Promise.all(
      agents.map((agent) =>
        memoryApi
          .spaceList({ agentId: agent.agent_id })
          .then((list) => [agent.agent_id, list.length] as const)
          .catch(() => [agent.agent_id, 0] as const),
      ),
    )
      .then((entries) => {
        if (!cancelled) setSpaceCounts(Object.fromEntries(entries));
      })
      .finally(() => {
  ...
  return { spaceCounts, spaceLoading };
}