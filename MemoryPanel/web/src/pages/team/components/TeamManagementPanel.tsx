...
export default function TeamManagementPanel({
  ...
  isAdmin: boolean;
  /**
   * 当前要管理的组织 ID（显式传入，来自 URL 参数）。
   * 关键：不再读写全局 activeTeamId —— 进入组织详情不应影响其他模块
   * （任务看板 / 资产 / 记忆空间 / Chat_Memory）的选中范围。
   */
  teamId: string;
  /**
  ...
  // 显式 teamId → 从 teams 列表解析出 activeTeam；不依赖全局 activeTeamId
  const activeTeam = useMemo(() => teams.find((t) => t.team_id === teamId) ?? null, [teams, teamId]);
  ...
  async function handleCreateAgent(card: Omit<AgentCard, 'id' | 'icon' | 'accent'>) {
    ...
    try {
      const created = await agentsApi.create(activeTeamId, {
        ...
        // 后端 resolveCreateVisibility：只有 system_admin 能建 team 公共 agent，
        // 普通成员只能建 private（仅自己 owner 可见）。前端按同一规则收敛，
        // 避免普通用户建 agent 时因硬编码 team 被 403「没有权限执行此操作」。
        visibility: _isAdmin ? 'team' : 'private',
      });
  ...
  async function handleCreateTeam(input: { name: string; description: string }) {
    ...
    try {
      ...
      // 创建后直接进入新组织详情；不改全局 activeTeamId
  ...
  async function handleDeleteTeam() {
    ...
    const ok = await tea.confirm({
      message: t('team.deleteTeam.confirm', { name: activeTeam.name }),
      description: t('team.deleteTeam.desc', { id: activeTeam.team_id }),
      okText: t('common.delete'),
    });
    if (!ok) return;
    setBusy(true);
    try {
      ...
      invalidateBackendCache();
      await refreshTeams();
      tea.notify.success(t('team.deleteTeam.success', { name: activeTeam.name }));
      // 删除的是当前正在查看的组织 → 返回组织列表
      navigate('/team');
    } catch (err) {
      tea.notify.error(errMsg(err));
    } finally {
  ...
  return (
    <div className="_memory-team-mgmt">
      {/* === Header: 当前 team 概览 + ops ===
        切 team 的入口在团队列表页（/team），这里不再提供
        平铺 chips 的切换入口，避免跟团队列表页形成两个语义重叠的控件。
        本卡片只承担三件事：
