...
import ScenePlaybookPanel from './ScenePlaybookPanel';
import BrainPanel from './BrainPanel';
import { ResultView } from '@/components/MemoryResultView';

// ── 通用展示块（可读结果渲染，替代裸 JSON）──
...
// ── Tab 1：记忆空间（四维 AND 组合筛选 + 真实挂载空间列表） ──
function SpacesPanel() {
  ...
  const activeTeam = useMemo(
    () => teams.find((tm) => tm.team_id === effectiveTeamId) ?? null,
  ...
  return (
    <Card>
      <Card.Body title={t('memoryCap.spaces.title')}>
        <div className="_asset-filter-quad" style={{ marginBottom: 12 }}>
          <Select
            appearance="button"
            matchButtonWidth
            clearable
        ...
        {busy ? (
          <div style={{ padding: 16, color: '#888' }}>{t('memoryCap.spaces.query')}…</div>
        ) : (
          <Table
            records={spaces ?? []}
            recordKey="id"
...
export default function MemoryCapabilitiesPanel() {
  ...
  const tabs = useMemo(
    () => [
      ...
      { id: 'scene', label: '场景/剧本' },
      { id: 'brain', label: 'Brain 读写' },
  ...
  return (
    <div style={{ padding: '0 0 24px' }}>
      <Tabs tabs={tabs}>
        ...
        <TabPanel id="scene"><ScenePlaybookPanel /></TabPanel>
        <TabPanel id="brain"><BrainPanel /></TabPanel>
