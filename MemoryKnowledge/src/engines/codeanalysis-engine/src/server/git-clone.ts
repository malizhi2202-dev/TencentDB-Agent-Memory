...
/**
 * 用户显式提供的 Git 凭据（TDAI 本地扩展）。
 * - password：HTTPS 用户名/密码（任意 Git 平台）
 * - token：HTTPS PAT/token（任意 Git 平台，username 缺省 oauth2）
 * - ssh：SSH 私钥（secret=私钥内容，passphrase 可选）
 * 凭据只经环境变量/临时文件传递，永不进 argv 或 URL。
 */
...
export async function cloneOrPull(
  ...
  options?: { token?: string; auth?: GitAuth },
): Promise<string> {
  ...
  if (exists) {
    ...
    await runGit(['pull', '--ff-only'], safeTarget, { token: options?.token, auth: options?.auth, url });
  } else {
    await fs.mkdir(path.dirname(safeTarget), { recursive: true });
    onProgress?.({ phase: 'cloning', message: `Cloning ${url}...` });
    await runGit(buildCloneArgs(url, safeTarget), undefined, { token: options?.token, auth: options?.auth, url });
  }
