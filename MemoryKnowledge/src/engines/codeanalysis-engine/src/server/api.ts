...
/**
 * Validate the optional `auth` field of POST /api/analyze (TDAI local
 * extension). Returns an { status, error } to send, or null when absent or
 * valid. Unlike the GitHub-bound `token`, `auth` is an explicit user-supplied
 * credential (password/token/ssh) honored on ANY Git host — validateGitUrl
 * already blocks private/internal-network hosts, and the credential is only
 * ever passed via env vars / temp files, never argv or the URL.
 *
 * Exported for unit tests.
 */
export function validateAnalyzeGitAuth(
  repoGitAuth: unknown,
): { status: number; error: string } | null {
  if (repoGitAuth === undefined || repoGitAuth === null) return null;
  if (typeof repoGitAuth !== 'object' || Array.isArray(repoGitAuth))
    return { status: 400, error: '"auth" must be an object' };
  const a = repoGitAuth as Record<string, unknown>;
  if (a.kind !== 'password' && a.kind !== 'token' && a.kind !== 'ssh')
    return { status: 400, error: '"auth.kind" must be "password", "token" or "ssh"' };
  if (typeof a.secret !== 'string' || a.secret.length === 0 || a.secret.length > 32768)
    return { status: 400, error: '"auth.secret" must be a non-empty string (max 32768)' };
  if (a.username !== undefined && (typeof a.username !== 'string' || a.username.length > 256))
    return { status: 400, error: '"auth.username" must be a string (max 256)' };
  if (a.passphrase !== undefined && (typeof a.passphrase !== 'string' || a.passphrase.length > 1024))
    return { status: 400, error: '"auth.passphrase" must be a string (max 1024)' };
  // ssh auth carries a PEM/OpenSSH private key: reject anything else that
  // could smuggle extra behavior into the temp key file.
  if (a.kind === 'ssh' && !(a.secret as string).includes('PRIVATE KEY'))
    return { status: 400, error: '"auth.secret" for ssh must be a PEM/OpenSSH private key' };
  return null;
}
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        // Auth: optional explicit credential (password/token/ssh) for any
        // Git platform. validateGitUrl already blocks private/internal hosts.
        const gitAuthError = validateAnalyzeGitAuth(repoGitAuth);
        if (gitAuthError) {
          res.status(gitAuthError.status).json({ error: gitAuthError.error });
          return;
        }
        const repoGitAuthTyped = repoGitAuth as GitAuth | undefined;
          ...
          try {
            ...
            if (repoUrl && !repoLocalPath) {
              ...
              await cloneOrPull(
                ...
                repoToken || repoGitAuthTyped
                  ? { token: repoToken, auth: repoGitAuthTyped ?? undefined }
                  : undefined,
              );
            }
